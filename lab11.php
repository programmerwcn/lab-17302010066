...
<form id="f_upload" action="lab11_save.php" method="post" enctype="multipart/form-data">
    <p>请上传音乐文件</p>

    <!--TODO: 在这里补充 html 元素，使 file_upload 上传后若为音乐文件，则可以直接播放-->
<div id="play"></div>
    <input type="file" name="file_upload" onchange="playMusic()">
    <table>
        ...
        <tr><td><input type="button" value="插入时间标签" onclick="insertTag()"></td><td><input type="button" value="替换时间标签"></td></tr>
...
for($i=0;$i<sizeof($a);$i++){
    $str='';
    $str=$str.'<option onclick="play('.$a[$i].')">'.$a[$i].'</option>';
    echo $str;
}
?>
...
var path=document.getElementById("upload_file").value;
    ...
    reader.onloadend=function(){
        ...
        if(path.match(".mp3")||path.match(".mp4")) {
            var playAudio = document.getElementById("play");
            srcOfAudio = reader.result;
            playAudio.innerHTML = '<audio id="playAudio" src="' + srcOfAudio + '" controls autoplay> </audio>';
        }//显示文件
...
function insertTag() {
    ...
    var changeTime=time/3600+":"+(time-time/3600)/60+":"+time%60;
var startPos=get_target_pos();
var endPos=startPos+changeTime.length;
var myfield=document.getElementById("edit_lyric");
myfield.value=myField.value = myField.value.substring(0, startPos) + changeTime + myField.value.substring(endPos, myField.value.length);
}
...
function play(path) {
    document.getElementById("play").innerHTML='<audio autoplay controls src="'+path+'"></audio>';
}
